  ...
  for _, img in ipairs(dt.collection) do
    for _,altimg in pairs(alts) do
      if string.find(img.filename, altimg) then
        table.insert(altcode, make_image_name(img, tmp_img_fn))
      end
    end
  end
...
local function register_storage_store(_, image, _, tmp_exp_path, _, _, _, _)
  ...
  if success then
    msgout(_("exported ") .. imagename) -- that is the path also
  else
    print(_('Upload failed: ') .. imagename)
    msgout(_("Failed to export ") .. imagename)
  end
end
...
local function register_storage_finalize(_, image_table, extra_data)
  ...
  msgout(_("exported ") .. fcnt .. "/" .. extra_data["init_img_cnt"] .. _(" images to Wikimedia Commons"))
end
...
local function register_storage_initialize(_, _, images, _, extra_data)
  ...
  for _, img in pairs(images) do
    if img.rights == "" then
      ...
      msgout(_("Error: ") .. img.path .. _(" has no rights, cannot be exported to Wikimedia Commons"))
