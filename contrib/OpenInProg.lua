...
Install: (see here for more detail: https://github.com/darktable-org/lua-scripts )
 ...
 Use the configurator found in the bottom left of lightable view to setup each program and what controls should be generated for each button
 In the top entry box enter the number of programs you would like to configure, this can be increased or decreased at any time by entering a new number and hitting refresh
 All settings will be saved and auto-loaded and controls generated at dt startup
 If you add new programs you can click the 'generate' button to create the new control buttons immediately for all newly added programs (avoiding having to restart dt)
 If you want to retain settings for a program, but don't want the controls generated you can de-select the 'enable' option and this program will be bypassed whenever generating controls (either at startup or via the button)
...
gettext.bindtextdomain("OpenInProg",dt.configuration.config_dir.."/lua/locale/")
local function _(msgid)
    return gettext.dgettext("OpenInProg", msgid)
end
...
local function NewProgram(index) -- creates a new program object and initializes it with values found in preferences for the program specified by index, generate all gui elements for that program as well
    ...
    function this:manage_sensitivity() --to ensure all options are stored to the appropriate preference lock out all options until the name is saved. if the name is changed update all preferences with the new name, also update the stack list chooser
        ...
        if self.GUI.change_name.label == _('change name') then --user requested to change program name, lock all other controls
            sense = false
            self.GUI.change_name.label = _('save name')
        elseif name_is_valid then --user requested to save new name and the new name is valid, update name, save all settings to pref with new name, update the program chooser list, unlock all other controls
            sense = true
            self.GUI.change_name.label = _('change name')
            ...
            sense = false
            dt.print(_('new name is not valid'))
        end
    ...
    this.GUI.open_all = dt.new_widget('check_button'){
        label = _('create "open all" button'), 
        value = dt.preferences.read(mod, this.name..'openAll', 'bool'),
        tooltip =_('create a button that will enable the "open all" behavior for specified program'),
        sensitive = false,
    ...
    this.GUI.open_each = dt.new_widget('check_button'){
        label = _('create "open each" button'), 
        value = dt.preferences.read(mod, this.name..'openEach', 'bool'),
        tooltip =_('create a button that will enable the "open each" behavior for specified program'),
        sensitive = false,
        clicked_callback = function(self) 
    ...
    this.GUI.export_all = dt.new_widget('check_button'){
        label = _('create "export all" button'), 
        value = dt.preferences.read(mod, this.name..'exportAll', 'bool'),
        tooltip =_('create a button that will enable the "export all" behavior for specified program'),
        sensitive = false,
        clicked_callback = function(self) 
    ...
    this.GUI.enable = dt.new_widget('check_button'){
        ...
        tooltip = _('generate this programs specified actions on startup or when generate is clicked'),
        sensitive = false,
        clicked_callback = function(self) 
    ...
    this.GUI.source_loc = dt.new_widget('check_button'){
        label = _('save to source location'),
...
GUI.show_programs = dt.new_widget('combobox'){
    label = _("show program"),
    tooltip = _("show options for specified program"),
    changed_callback = function(self)
...
local function UpdateProgramList() -- updates the stack to contain only the number of programs specified, updates program chooser list as well
    local temp = #programs
    local qty_programs = math.max(tonumber(GUI.qty_prog_entry.text),0)
...
local function pre_call(prog) --checks if program exists (only on initial call), returns true if there is an install error
    if prog.first_run then
        ...
        if not prog.bin then
            dt.print_error(prog.name.._(' not found'))
            dt.print(_('ERROR - ')..prog.name.._(' not found'))
...
local function Func_OpenAll(ButtonText) --parses button text to determine what program object to use while executing, attempts to open all images in a single call
    ...
    if pre_call(programs[idx]) then
        dt.print(_('resolve install issue with ')..programs[idx].name)
        return
    end
    
    dt.print(_('opening all in ')..programs[idx].name)
    ...
    if resp ~= 0 then
        dt.print_error(_('an error occured while trying to open images in ')..programs[idx].name)
    end
end
...
local function Func_OpenEach(ButtonText) --parses button text to determine what program object to use while executing, attempts to open each image in a unique call
    ...
    dt.print(_('opening each in ')..programs[idx].name)
...
local function Func_Export(storage, image_table, extra_data) --uses storage name to determine what program object to use while executing, attempts to export all images, move them to designated folder, then open those all in a single call
    ...
    dt.print(_("Opening exported images in ")..programs[idx].name)
    local images_to_open = ''
...
GUI.refresh = dt.new_widget('button'){
    label = _('refresh'),
    tooltip = _('refresh list of programs available'),
    clicked_callback = function() UpdateProgramList() end
}

GUI.generate = dt.new_widget('button'){
    label =  _('generate controls'),
    tooltip = _('generate buttons for all programs. If new programs have been added they will be generated now, if you have removed programs they will no longer appear after a restart of dt'),
...
dt.register_lib(
    mod,
    _('Open In Program Configurator'),
    true,
    false,
