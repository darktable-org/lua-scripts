...
--  dt.print(_(string.format("Exporting to gimp %i/%i", number, total)))
end
...
local function gimp_edit(storage, image_table, extra_data) --finalize
  ...
  for _,exp_img in pairs(image_table) do
    ...
    dt.print_error(fname)
    for _,orig_img in pairs(dt.gui.action_images) do
      if basename(fname) == basename(orig_img.filename) then
        dt.print_error("moving file...")
        local myimage_name = orig_img.path .. "/" .. fname
        dt.print_error(myimage_name)
        result = os.rename(exp_img, myimage_name)
